...
--加载数据到红餐厅
load data local inpath '/home/recomend/res/*' overwrite into table red_res;  


--加载数据到res_mapping
load data local inpath '/home/recomend/res_mapping/*' overwrite into table res_mapping;
...
--上传数据至hive数据仓库
load data local inpath '/home/recomend/order/*' overwrite into table order_scores ;  

	
--插入数据到   order_scores_sum

insert overwrite table order_scores_sum select memberid,rescharc,max(order_num_score) from order_scores group by memberid,rescharc ;


--插入数据到  scores

--insert overwrite table scores select p.memberid,m.resid,p.view_num_score+p.view_time_score+o.order_num_score as score from  iis_pageview_scores p inner join red_res r on p.rescharc = r.rescharc inner join res_mapping_distinct m on p.rescharc = m.rescharc inner join order_scores_sum o where p.memberid=o.memberid and p.rescharc = o.rescharc;


insert overwrite table scores select o.memberid,m.resid,if(p.view_num_score is null,0,p.view_num_score)+if(p.view_time_score is null,0,p.view_time_score)+o.order_num_score as score from  order_scores_sum o left outer join iis_pageview_scores p on o.memberid = p.memberid and o.rescharc = p.rescharc inner join red_res r on o.rescharc = r.rescharc inner join res_mapping_distinct m on o.rescharc = m.rescharc;
